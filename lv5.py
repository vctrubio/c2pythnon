...
def beeramid(bonus, price):
    ...
    while total_cans_used <= cans:
        ...
        total_cans_used += max(0, level**2)
    return 0 if level == 0 else level - 1


# diff in ip address (IPv4)
def ips_between(start, end):
    ip1_list = list(map(int, start.split('.')))
    ip2_list = list(map(int, end.split('.')))
    print(ip1_list)
    print(ip2_list)

    result = 0
    for i in range(4):
        result = result * 256 + (ip2_list[i] - ip1_list[i])
